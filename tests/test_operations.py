...
def test_guardian_compression_hands_a_compatible_bundle_to_either_transport(
    ...
    operations, workers, mailstore = _operations(
        tmp_path, guardian_compression=True, payload_backend="ofdm_vhf"
    )
    mail = MailMessage(
...
def test_vara_files_compression_replaces_but_does_not_stack_on_text(tmp_path) -> None:
    operations, workers, _ = _operations(
...
def test_live_settings_persist_vara_encryption_for_the_next_modem_start(tmp_path) -> None:
    executable = tmp_path / "VARAFM.exe"
    executable.touch()
    operations, workers, _ = _operations(
        tmp_path,
    ...
    operations.vara = _FakeVara()
    try:
        assert operations.apply_vara_encryption_setting()
        ini = executable.with_suffix(".ini").read_text(encoding="utf-8")
        assert "Encryption=1" in ini
        assert "Password encryption=SharedKey2026" in ini
    finally:
        operations.close()
        workers.close(wait=True)
