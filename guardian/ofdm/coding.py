...
def _ldpc_graph(information: int, parity: int) -> _LdpcGraph:
    ...
    strides = (1, 5, 11)
    offsets = (0, max(1, parity // 3), max(2, 2 * parity // 3 + 1))
    for variable in range(information):
        chosen: list[int] = []
        for stride, offset in zip(strides, offsets, strict=True):
            check = (variable * stride + offset) % parity
            while check in chosen:
                check = (check + 1) % parity
            chosen.append(check)
            checks[check].append(variable)
        data_checks[variable] = chosen
...
def _ldpc_decode(soft: np.ndarray, information: int, rate: Fraction,
                 max_iterations: int = 24) -> np.ndarray:
    ...
    for _ in range(max(1, int(max_iterations))):
        for check in range(parity):
            first = int(graph.check_starts[check])
            last = int(graph.check_starts[check + 1])
            incoming = variable_to_check[first:last]
            ...
            second = (float(np.partition(magnitudes, 1)[1])
                      if len(magnitudes) > 1 else minimum)
            signs = np.where(incoming >= 0.0, 1.0, -1.0)
            combined = float(np.prod(signs))
            outgoing = np.full(len(incoming), minimum, dtype=np.float64)
            outgoing[minimum_index] = second
            check_to_variable[first:last] = 0.80 * outgoing * combined * signs
        sums = np.bincount(
        ...
        for check in range(parity):
            first = int(graph.check_starts[check])
            last = int(graph.check_starts[check + 1])
            if int(np.bitwise_xor.reduce(hard[graph.edge_var[first:last]])):
                valid = False
                break
        if valid:
...
def iterative_decode_candidates(soft, byte_count: int,
                                ...
                                iterations: int = 3) -> list[np.ndarray]:
