"""Deterministic benchmark helpers for the production SC-FTN waveform."""
...
def write_wav(path: Path | str, samples, sample_rate: int) -> Path:
    """Write mono 16-bit PCM, the format used by the SC diagnostic tools."""
    path = Path(path)
...
def read_wav(path: Path | str) -> tuple[np.ndarray, int]:
    """Read mono 16-bit PCM into normalized float samples."""
...
def run_burst(profile: WaveformProfile, mcs_index: int = 2, *,
    ...
    return BurstResult(
        ...
        # A modulation-only closed form is misleading for FTN; the
